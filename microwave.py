...
def reset_globals():
    ...
    global target_queue, math_save_queue, metadata_queue, capture_done_event, stop_event, restart_requested, activity_timestamp
    update_activity() # Use your existing thread-safe function
...
N_AVG_SKY = 150                 
N_AVG_CAL = 12
window = np.hanning(NSAMPLES).astype(np.float32)[None,None,:]
    ...
    def internal_math_thread():
        while True:
            ...
            try:
                ...
                x = (reshaped[...,0].astype(np.float32) +
                1j * reshaped[...,1].astype(np.float32)).astype(np.complex64)

                means = x.mean(axis=2, keepdims=True)
                x -= means
                x *= window

                fft = np.fft.fft(x, axis=2)

                np.multiply(fft.real, fft.real, out=fft.real)
                np.multiply(fft.imag, fft.imag, out=fft.imag)
                fft.real += fft.imag

                result = fft.real.mean(axis=(0,1))
    ...
    while True:
        ...
        if cmd["action"] == "stop":
            ...
            try:
                ...
                massive_capture = np.empty((total_blocks, NSAMPLES, 2), dtype=np.int8)

                i = 0
                while i < total_blocks:
                    take = min(chunk_blocks, total_blocks - i)

                    chunk = sdr.capture_data(NSAMPLES, take)
                    massive_capture[i:i+take] = chunk
...
if __name__ == "__main__":
    mp.set_start_method('fork', force=True) # Recommended for robust SDR multiprocessing
    main()
