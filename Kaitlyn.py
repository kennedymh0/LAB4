...
def capture_and_average(nobs, sdr, result_queue):
    ...
    try:
        ...
        # Pre-allocate a single array for our running sum
        sum_spec = np.zeros(NSAMPLES, dtype=np.float64)
        ...
        for i in range(nobs):
            ...
            # Apply window (numpy broadcasts this automatically to the last axis)
            x_complex *= window
            
            # Calculate the FFT over the last axis (NSAMPLES) explicitly for speed
            spec = np.abs(np.fft.fftshift(np.fft.fft(x_complex, axis=-1), axes=-1)) ** 2
            
            # Accumulate the mean of the blocks into our running sum
            sum_spec += np.mean(spec, axis=0)
            
        # Divide by total observations to get the final average
        result_queue.put(sum_spec / nobs)
...
def writer_thread():
    ...
    while not stop_event.is_set() or not save_queue.empty():
        try:
            ...
            # Only write to disk every 5 observations, or if it's the very last item in the queue
            if save_counter % 5 == 0 or (stop_event.is_set() and save_queue.empty()):
                np.savez(OUTFILE, data=np.array(all_data, dtype=object))
                print(f"💾 Saved {len(all_data)} total observations to disk.")
