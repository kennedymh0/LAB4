...
def sdr_worker(device_index, cmd_queue, res_queue, math_res_queue):
    ...
    def internal_math_thread():
        # Cast the global window to float32 locally!
        # This prevents numpy from accidentally up-casting our math back to massive 64-bit arrays.
        local_window = window.astype(np.float32)
        ...
        while True:
            ...
            try:
                ...
                # Use float32 to save memory and double performance
                running_sum = np.zeros(NSAMPLES, dtype=np.float32)
                
                # Chunk size for math: Process blocks in batches (e.g. 2500 blocks at a time).
                # This keeps memory footprint tiny while maximizing Numpy's C-level vectorization.
                CHUNK_SIZE = 2500 
                
                for i in range(0, total_blocks, CHUNK_SIZE):
                    raw_chunk = massive_capture[i : i + CHUNK_SIZE]
                    
                    # 1. Vectorized conversion to complex64 
                    x_complex = raw_chunk[..., 0].astype(np.float32) + 1j * raw_chunk[..., 1].astype(np.float32)
                    
                    # 2. Vectorized DC removal across the samples axis
                    x_complex -= np.mean(x_complex, axis=1, keepdims=True)
                    
                    # 3. Apply window 
                    x_complex *= local_window
                    
                    # 4. Vectorized batched FFT & Power calculation 
                    spec = np.abs(np.fft.fftshift(np.fft.fft(x_complex, axis=1), axes=1)) ** 2
                    
                    # 5. Sum the batch's spectra (axis=0) into our running total
                    running_sum += np.sum(spec, axis=0)
                    
                # Divide by total blocks to get the final mean spectrum
                result = running_sum / total_blocks
                ...
                del massive_capture
                if 'raw_chunk' in locals(): del raw_chunk
                if 'x_complex' in locals(): del x_complex
    ...
    while True:
        ...
        if cmd["action"] == "stop":
            ...
            try:
                ...
                # CONTINUOUS CAPTURE: Do not loop this. Grabbing it all at once keeps
                # the SDR's async hardware stream open, preventing USB timeouts/crashes.
...
def run_observation_session(l_start, b_start, use_missing=False):
    ...
    if use_missing:
        from hvc_missing import get_missing_pointings
...
def main():
    ...
    if ans == "y":
        ...
        # the session will ignore these and pull from the missing list anyway.
        l_start, b_start = 0.0, 0.0 
    else:
    ...
    while True:
        # Pass the use_missing flag into the session
        needs_restart, l_start, b_start = run_observation_session(l_start, b_start, use_missing)
