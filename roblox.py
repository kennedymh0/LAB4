...
math_save_queue = queue.Queue(maxsize=10)        # Passes reduced data to the background writer thread
...
def sdr_worker(device_index, cmd_queue, res_queue, math_res_queue):
    ...
    logging.basicConfig(
    level=logging.DEBUG,
    format="[WORKER %(process)d] %(asctime)s %(message)s",
    datefmt="%H:%M:%S",
    stream=sys.stdout)
    ...
    def internal_math_thread():
        ...
        while True:
            ...
            logging.info(f"FFT start ({task_type})")
            ...
            try:
                ...
                logging.info(f"FFT done ({task_type})")
                del massive_capture
    ...
    while True:
        ...
        # 💓 HEARTBEAT (PUT IT HERE)
            res_queue.put({"status": "heartbeat"})
            continue
        ...
        if cmd["action"] == "stop":
            ...
            try:
                ...
                # CRITICAL: Drop the main thread's reference so the math thread 
                # is the sole owner. This allows GC to fire immediately when 
                # the math thread is done, keeping peak RAM at 1.2GB instead of 2.4GB.
                del massive_capture 
...
def watchdog_thread(p0, p1):
    ...
    while not stop_event.is_set():
        ...
        for name, t_last in heartbeat.items():
            if now - t_last > TIMEOUT:
                log.error(f"🚨 WATCHDOG: {name} is stalled for {now - t_last:.1f}s")
                stop_event.set()
                return

        if not p0.is_alive():
            log.error("🚨 WATCHDOG: worker 0 died")
            stop_event.set()
            return

        if not p1.is_alive():
            log.error("🚨 WATCHDOG: worker 1 died")
            stop_event.set()
            return

        log.info(
            f"[QUEUES] "
            f"target={target_queue.qsize()} | "
            f"meta={metadata_queue.qsize()} | "
            f"save={math_save_queue.qsize()}"
        )
...
def observer_thread(cmd_q0, cmd_q1, res_q0, res_q1, noise):
    while not stop_event.is_set():
        ...
        try:
            ...
            noise.off()
            start = time.time()
            log.info(f"[OBSERVER] Sky capture start l={l_deg}, b={b_deg}")

            cmd_q0.put({"action": "capture", "nobs": N_AVG_SKY, "type": "sky"})
            cmd_q1.put({"action": "capture", "nobs": N_AVG_SKY, "type": "sky"})

            try:
                def wait_for_capture(res_q, worker_name):
                    while True:
                        msg = res_q.get(timeout=30)

                        if msg["status"] == "heartbeat":
                            heartbeat[worker_name] = time.time()
                            continue

                    return msg
                msg0 = wait_for_capture(res_q0, "worker0")
                msg1 = wait_for_capture(res_q1, "worker1")
            except queue.Empty:
                log.error("🚨 Sky capture timeout")
                raise

            log.info(f"[OBSERVER] Sky capture done in {time.time()-start:.2f}s")

            # --- Cal Capture ---
            noise.on()
            start = time.time()
            log.info(f"[OBSERVER] Cal capture start l={l_deg}, b={b_deg}")

            cmd_q0.put({"action": "capture", "nobs": N_AVG_SKY, "type": "cal"})
            cmd_q1.put({"action": "capture", "nobs": N_AVG_SKY, "type": "cal"})

            try:
                msg0 = res_q0.get(timeout=30)
                msg1 = res_q1.get(timeout=30)
            except queue.Empty:
                log.error("🚨 Sky capture timeout")
                raise

            log.info(f"[OBSERVER] Cal capture done in {time.time()-start:.2f}s")
            noise.off()

            # SIGNAL TELESCOPE TO MOVE IMMEDIATELY
            if not capture_done_event.wait(timeout=120):
                print("⚠️ Timeout waiting for capture_done_event")
                stop_event.set()
            target_queue.task_done()

            # --- Pass off to Collector (NO BLOCKING!) ---
            metadata_queue.put({
                "time": time.time(),
...
def writer_thread():
    ...
    while not stop_event.is_set() or not math_save_queue.empty():
        heartbeat["writer"] = time.time()
        log.info(f"[WRITER] Queue size: {math_save_queue.qsize()}")
        try:
...
def collector_thread(math_res_q0, math_res_q1):
    """Waits for background math so the observer thread doesn't have to."""
    
    while not stop_event.is_set():
        heartbeat["collector"] = time.time()
        try:

            # Get the coordinates passed from the observer
            log.info("[COLLECTOR] Waiting for metadata")
            meta = metadata_queue.get(timeout=10)
            log.info(f"[COLLECTOR] Got metadata l={meta['l']}, b={meta['b']}")
...
def main():
    ...
    threads = [
        ...
        threading.Thread(target=collector_thread, args=(math_res_q0, math_res_q1), daemon=True), # NEW
        threading.Thread(target=writer_thread, daemon=True),
        threading.Thread(target=watchdog_thread, args=(p0, p1), daemon=True)
    ]
    ...
    try:
        while any(t.is_alive() for t in threads):
            if not p0.is_alive() or not p1.is_alive():
                print("🚨 Worker died!")
                stop_event.set()
                break
            time.sleep(1)
