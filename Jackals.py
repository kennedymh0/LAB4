...
def sdr_worker(device_index, cmd_queue, res_queue):
    ...
    while True:
        ...
        if cmd["action"] == "stop":
            ...
            try:
                for _ in range(nobs):
                    # 1. Read a tiny 2MB chunk synchronously (Highly stable, no asyncio leaks!)
                    chunk_bytes = sdr.read_bytes(NBLOCKS * NSAMPLES * 2)
                    
                    # 2. Reshape exactly like the old function
                    chunk = (np.frombuffer(chunk_bytes, dtype='uint8') - 128).view('int8')
                    chunk.shape = (NBLOCKS, NSAMPLES, 2)
                    
                    # 3. Process the FFT immediately to free RAM
                    x_complex = chunk[..., 0] + 1j * chunk[..., 1]
                ...
                # Average the result
                final_spectrum = running_sum / nobs
                
                # Send ONLY the final 8KB array back to the main process
                res_queue.put({"status": "ok", "data": final_spectrum})
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            math_save_queue.put({
                ...
                "freq": freqs,
                "power0": res0["data"], "power1": res1["data"],
                "pcal0": res_cal0["data"], "pcal1": res_cal1["data"]
            })
